...
bool HttpClient::ParseHeaderLine(const std::string& line) {
    ...
    // A response may carry several Set-Cookie headers; keep them all (joined by
    // '\n') instead of letting the last one overwrite the rest, so callers can
    // recover every cookie (e.g. zingmp3.vn issues zmp3_app_version + zmp3_rqid).
    if (lower_key == "set-cookie") {
        auto existing = response_headers_.find(lower_key);
        if (existing != response_headers_.end()) {
